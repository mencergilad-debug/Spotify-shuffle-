...
async function handleCallback() {
  ...
  if (!code) return;
  if (state !== localStorage.getItem('spotify_auth_state')) throw new Error('Login state mismatch. Try connecting again.');
...
async function refreshAccessTokenIfNeeded() {
  ...
  const next = { ...t, ...data, refresh_token: data.refresh_token || t.refresh_token, expires_at: Date.now() + data.expires_in * 1000 };
  setTokenData(next);
...
async function getAllSavedTrackUris() {
  ...
  while (url) {
    ...
    for (const item of data.items) if (item.track && item.track.uri) uris.push(item.track.uri);
    url = data.next;
...
async function getMe() { return await spotify('/me'); }
...
async function findOrCreatePlaylist(userId) {
  ...
  return await spotify(`/users/${encodeURIComponent(userId)}/playlists`, {
    method: 'POST',
    body: JSON.stringify({ name: PLAYLIST_NAME, public: false, description: 'Generated by Spotify True Shuffle. Play with Spotify shuffle OFF.' }),
  });
}
...
async function replacePlaylist(playlistId, uris) {
  ...
  await spotify(`/playlists/${playlistId}/tracks`, { method: 'PUT', body: JSON.stringify({ uris: groups[0] || [] }) });
  setProgress(60);
  for (let i = 1; i < groups.length; i++) {
    await spotify(`/playlists/${playlistId}/tracks`, { method: 'POST', body: JSON.stringify({ uris: groups[i] }) });
...
els.saveClientId.addEventListener('click', () => { setClientId(els.clientId.value); setStatus('Client ID saved. Now connect Spotify.'); updateUi(); });
els.loginBtn.addEventListener('click', async () => { try { await login(); } catch (e) { setStatus(e.message); } });
els.refreshBtn.addEventListener('click', async () => { try { await refreshTrueShuffle(); } catch (e) { setStatus(`Error: ${e.message}`); els.refreshBtn.disabled = false; } });
els.logoutBtn.addEventListener('click', () => { clearTokenData(); setStatus('Disconnected.'); updateUi(); });

(async function init() {
  try { await handleCallback(); } catch (e) { setStatus(`Login error: ${e.message}`); }
  updateUi();
})();
